...
public class FactIntentHandler implements RequestHandler {
    ...
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName("FactIntent"));
    }

    @Override
    public Optional<Response> handle(HandlerInput input) {

        List<String> listaJuegos = new ArrayList<String>();
        listaJuegos.add(
                "Hasta el siglo XVI, las multiplicaciones se consideraban tan difíciles que sólo se enseñaban en las universidades.");
        listaJuegos.add("Se afirma que hay un patron escondido para los numero primos");

        if (listaJuegos.size() <= 0) {
            keys = FactsUtil.getKeys();
        }

        int index = new Random().nextInt(listaJuegos.size());
        String curiosidad = listaJuegos.get(index);
        listaJuegos.remove(index);

        String title = "¿Sabias qué?";
        String primaryText = facts.get(curiosidad);
        // FIXME: If you would like to display additional text, please set the secondary
        // text accordingly
        String secondaryText = "";
        String speechText = "<speak> " + primaryText
                + "<break time=\"1s\"/>  Would you like to hear another airplane fact?" + " </speak>";
        ...
        if (null != input.getRequestEnvelope().getContext().getDisplay()) {
            return input.getResponseBuilder().withSpeech(speechText).withSimpleCard(title, primaryText)
                    .addRenderTemplateDirective(template).withReprompt(speechText).build();
        } else {
            // Headless device
            return input.getResponseBuilder().withSpeech(speechText).withSimpleCard(title, primaryText)
    ...
    private Template getBodyTemplate3(String title, String primaryText, String secondaryText, Image image) {
        return BodyTemplate3.builder().withImage(image).withTitle(title)
                .withTextContent(getTextContent(primaryText, secondaryText)).build();
    }
    ...
    /**
     * Helper method that returns the rich text that can be set as the text content
     * for a body template.
     * 
     * @param text The string that needs to be set as the text content for the body
     *             template.
     * @return RichText that will be rendered with the body template
     */
